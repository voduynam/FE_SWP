...
const menuItems = [
  { 
    ...
    path: '/app/dashboard',
    roles: ['admin', 'manager', 'central_kitchen', 'supply_coordinator', 'franchise_staff']
  },
  { 
    title: 'Đơn hàng', 
    icon: ClipboardList, 
...
const menuByRole = {
  admin: [
    ...
    { title: 'Cửa hàng & bếp trung tâm', icon: Store, path: '/app/admin/stores' },
  ...
  manager: [
    ...
    { title: 'Sản phẩm & công thức', icon: Package, path: '/app/manager/products' },
    { title: 'Tồn kho hệ thống', icon: Warehouse, path: '/app/manager/inventory' },
    { title: 'Báo cáo & hiệu suất', icon: BarChart3, path: '/app/manager/reports' },
  ],
  central_kitchen: [
    { title: 'Dashboard', icon: LayoutDashboard, path: '/app/central/dashboard' },
    { title: 'Đơn hàng từ cửa hàng', icon: ClipboardList, path: '/app/central/orders' },
    { title: 'Sản xuất & xuất kho', icon: TruckIcon, path: '/app/central/production' },
    { title: 'Nguyên liệu & lô sản xuất', icon: Warehouse, path: '/app/central/materials' },
  ],
  supply_coordinator: [
    { title: 'Dashboard', icon: LayoutDashboard, path: '/app/supply/dashboard' },
    { title: 'Tổng hợp đơn hàng', icon: ClipboardList, path: '/app/supply/orders' },
    { title: 'Điều phối & giao hàng', icon: TruckIcon, path: '/app/supply/delivery' },
    { title: 'Xử lý sự cố', icon: Settings, path: '/app/supply/issues' },
  ],
  franchise_staff: [
    { title: 'Dashboard', icon: LayoutDashboard, path: '/app/store/dashboard' },
    { title: 'Đơn hàng cửa hàng', icon: ClipboardList, path: '/app/store/orders' },
    { title: 'Tồn kho cửa hàng', icon: Warehouse, path: '/app/store/inventory' },
  ],
};
...
export default function Sidebar({ userRole, userName, onLogout }) {
          ...
          <ul className="space-y-1">
            {roleMenuItems.map((item) => (
              <li key={item.path}>
                ...
                >
                  <item.icon className={`w-5 h-5 flex-shrink-0 ${isActive(item.path) ? 'text-orange-400' : 'text-slate-300'}`} />
